...
bool buildFDRmapping(ifstream& ifs, vector<pair<int, long double> >& p_to_q)
{
  ...
  it--;
  int i = 0;
  long double prevFDR(-1.);
  long double FDR;
  long double N(static_cast<long double>(numPvalues));
  long double numThisExtremeOrMoreExtreme(0);
  while (it != tempMap.begin())
    {
      p_to_q[i].first = it->first;
      numThisExtremeOrMoreExtreme += static_cast<long double>(it->second);
      FDR = static_cast<long double>(powl(10., -it->first/CHANGE_OF_SCALE)) * N / numThisExtremeOrMoreExtreme;
      if (FDR < prevFDR)
	FDR = prevFDR;
      if (FDR > 0.999)
	{
  ...
  for (it = vec.begin(); it != vec.end(); it++)
      ...
      if (it->FDR <= FDRthreshold)
	{
	  cout << *prevChromString << '\t'
	       ...
	       << it->FDR;
	  if (writePvals)
	    cout << '\t' << powl(10., -it->negLog10P_scaled/CHANGE_OF_SCALE);
	  cout << '\n';
	}
    }
