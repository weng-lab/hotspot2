				...
				)
{
  ...
  if (pval < 0 || pval > 1)
    ...
    negLog10P_scaled = static_cast<int>(floorl(-log10(pval) * CHANGE_OF_SCALE + 0.5));
...
void BackgroundRegionManager::add(const SiteRange& s)
{
  ...
  //
  // m_nextPosToSample += static_cast<int>(ceill(static_cast<long double>(s.endPos - m_nextPosToSample)/static_cast<long double>(m_samplingInterval))) * m_samplingInterval;
  //
  ...
  if (0 == m_runningSum_count || 1 == m_runningSum_count) // the only way for count data to have m = v
  ...
  else
    {
      if (m < v)
        ...
        {
          // binomial
          long double n(floorl(m * m / (m - v) + 0.5)); // estimate n of the fit from the observed mean and variance
